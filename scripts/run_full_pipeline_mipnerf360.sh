...
CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" pip install -e "${RASTER_ORIG}" --no-build-isolation
...
pip uninstall -y diff-gaussian-rasterization >/dev/null 2>&1 || true
CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" pip install -e "${RASTER_FW}" --no-build-isolation

if [[ "${RUN_MINIMAL}" == "1" ]]; then
  echo "==> Run minimal A/B/C validation (uses FW rasterizer)"
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/validate_fw_minimal.py -s "${SRC}" -i "${IMAGES}" -m "${MINIMAL_OUT}" \
...
if [[ "${RUN_ORACLE}" == "1" ]]; then
  ...
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/validate_fw_oracle.py -s "${SRC}" -i "${IMAGES}" -m "${FW_OUT}" --iteration "${ITERATIONS}" \
...
if [[ "${RUN_ABLATION}" == "1" ]]; then
  ...
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/validate_fw_ablation.py -s "${SRC}" -i "${IMAGES}" -m "${FW_OUT}" --iteration "${ITERATIONS}" \
    --num_candidates 200 --out "${FW_OUT}/fw_ablation.json"
fi

if [[ "${RUN_PROFILE}" == "1" ]]; then
  echo "==> Profiling FW overhead"
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/profile_fw_overhead.py -s "${SRC}" -i "${IMAGES}" -m "${FW_OUT}" --iteration "${ITERATIONS}" \
    --iters 10 --out "${FW_OUT}/fw_overhead.json"
fi

if [[ "${RUN_BUDGET}" == "1" ]]; then
  echo "==> Budget-quality curves"
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/collect_budget_curve.py -s "${SRC}" -i "${IMAGES}" -m "${BASELINE_OUT}" --iterations ${BUDGET_ITERS} \
    --max_views "${BUDGET_MAX_VIEWS}" --out "${BASELINE_OUT}/budget_curve.json"
  CUDA_INVISIBLE_DEVICES="${CUDA_INVISIBLE_DEVICES}" python scripts/collect_budget_curve.py -s "${SRC}" -i "${IMAGES}" -m "${FW_OUT}" --iterations ${BUDGET_ITERS} \
