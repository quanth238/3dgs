...
def rasterize_gaussians_with_buffers(
    means3D,
    means2D,
    sh,
    colors_precomp,
    ...
    return _RasterizeGaussiansWithBuffers.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
...
class _RasterizeGaussiansWithBuffers(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means3D,
        ...
        ctx.num_rendered = num_rendered
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, radii, sh, opacities, geomBuffer, binningBuffer, imgBuffer)
        return color, radii, invdepths, geomBuffer, binningBuffer, imgBuffer

    @staticmethod
    def backward(ctx, grad_out_color, _, grad_out_depth, grad_geom, grad_bin, grad_img):
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        colors_precomp, means3D, scales, rotations, cov3Ds_precomp, radii, sh, opacities, geomBuffer, binningBuffer, imgBuffer = ctx.saved_tensors
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None, return_aux = False):
        ...
        if return_aux:
            return rasterize_gaussians_with_buffers(
                means3D,
                means2D,
                shs,
                colors_precomp,
